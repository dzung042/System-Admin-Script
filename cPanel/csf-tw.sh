#!/bin/bash
#####################################################
# advanced tuning for CSF firewall
...
#####################################################
DT=$(date +"%d%m%y-%H%M%S")
cp -a /etc/csf/csf.conf /etc/csf/csf.conf.bak.$(date +%F-%H%M%S)
denyiplimits() {
if [[ ! -f /proc/user_beancounters && -f /usr/sbin/ipset ]] && [[ "$(uname -r | grep linode)" || "$(find /lib/modules/`uname -r` -name 'ipset' >/dev/null 2>&1; echo $?)" -eq '0' ]] && [[ "$(virt-what | grep -o lxc)" != 'lxc' ]]; then
  ...
  if [[ "$CSFTOTALMEM" -ge '65000001' ]]; then
    ...
    grep -q '^LF_IPSET' /etc/csf/csf.conf \
    && sed -i 's/^LF_IPSET *= *.*/LF_IPSET = "1"/' /etc/csf/csf.conf \
    || echo 'LF_IPSET = "1"' >> /etc/csf/csf.conf
    
    grep -q '^LF_IPSET_HASHSIZE' /etc/csf/csf.conf \
    && sed -i 's/^LF_IPSET_HASHSIZE *= *.*/LF_IPSET_HASHSIZE = "4096"/' /etc/csf/csf.conf \
    || echo 'LF_IPSET_HASHSIZE = "4096"' >> /etc/csf/csf.conf
    
    grep -q '^LF_IPSET_MAXELEM' /etc/csf/csf.conf \
    && sed -i 's/^LF_IPSET_MAXELEM *= *.*/LF_IPSET_MAXELEM = "262144"/' /etc/csf/csf.conf \
    || echo 'LF_IPSET_MAXELEM = "262144"' >> /etc/csf/csf.conf
    
    sed -i 's/^WAITLOCK_TIMEOUT *= *.*/WAITLOCK_TIMEOUT = "30"/' /etc/csf/csf.conf
    sed -i 's/^PACKET_FILTER *= *.*/PACKET_FILTER = "0"/' /etc/csf/csf.conf
    sed -i 's/^LF_LOOKUPS *= *.*/LF_LOOKUPS = "0"/' /etc/csf/csf.conf
    sed -i 's/^DROP_LOGGING *= *.*/DROP_LOGGING = "0"/' /etc/csf/csf.conf
    sed -i 's/^DROP_OUT_LOGGING *= *.*/DROP_OUT_LOGGING = "0"/' /etc/csf/csf.conf
    sed -i 's/^DROP_UID_LOGGING *= *.*/DROP_UID_LOGGING = "0"/' /etc/csf/csf.conf
